...
void UNDIViewportCaptureComponent::ChangeCaptureSettings(FIntPoint InCaptureSize, FFrameRate InCaptureRate)
{
	...
	// Now comes the magic ...
	FTexture2DRHIRef ReferenceTexture = RHICreateTexture2D(CaptureSize.X, CaptureSize.Y, EPixelFormat::PF_B8G8R8A8, 1,
	...
	RHICreateTargetableShaderResource2D(CaptureSize.X, CaptureSize.Y, EPixelFormat::PF_B8G8R8A8, 1, TexCreate_Dynamic,
...
void UNDIViewportCaptureComponent::TickComponent(float DeltaTime, ELevelTick TickType,
			...
			if (ViewFamily != nullptr)
			{
				...
				if (number_of_connections > 0 && View != nullptr)
				{
					...
					if (UTextureRenderTarget2D* RenderTarget = NDIMediaSource->GetRenderTarget())
					{
						...
						// Ensure we empty the extensions, as they get populated within the call to
							...
							RenderTarget->Resource->TextureRHI->GetSizeXYZ() != RenderableTexture->GetSizeXYZ())
						{
							...
							([&](FRHICommandListImmediate& RHICmdList) {
								...
								if (UTextureRenderTarget2D* MediaTexture = NDIMediaSource->GetRenderTarget())
								{
									...
									RHIUpdateTextureReference(MediaTexture->TextureReference.TextureReferenceRHI,
...
FIntPoint UNDIViewportCaptureComponent::GetSizeXY() const
{
	return this->CaptureSize;
}

float UNDIViewportCaptureComponent::GetDisplayGamma() const
{
	return 2.2f;
}
